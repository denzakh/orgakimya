...
@foreach($localeFieldArray as $fieldName)
	...
	@foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $localeCodeProperties)
	    @php   
	    	...
	    	$inputValue = isset($application) ? $application[$inputName] : '';
	    @endphp 
...
<h2 class="pt-2">media</h2>
    ...
    <div>
	    <label for="">img</label>
		<input type="text" class="form-control" name="img" placeholder="" value="{{ $application['img'] }}" readonly> 
	<div>
    ...
    <div>
	    <label for="">doc</label>
		<input type="text" class="form-control" name="doc" placeholder="" value="{{ $application['doc'] }}" readonly> 
	<div>
